...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path))
        df_raw['tmp']=pd.to_datetime(df_raw['date']).dt.date
        df_raw=df_raw[df_raw['tmp']!=datetime.date(2018,6,26)]
        del df_raw['tmp']

        # border1s = [0, 12 * 30 * 24 - self.seq_len, 12 * 30 * 24 + 4 * 30 * 24 - self.seq_len]
        # border2s = [12 * 30 * 24, 12 * 30 * 24 + 4 * 30 * 24, 12 * 30 * 24 + 8 * 30 * 24]
        border1s = [0, 717 * 24 - self.seq_len, 721 * 24 - self.seq_len, 0]
        border2s = [717 * 24, 721 * 24, 725 * 24, 725 * 24]
        ...
        if self.timeenc == 0:
            ...
            data_stamp = df_stamp['date'].values
